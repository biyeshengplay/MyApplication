                    ...
                    if (tempJsonObject == null) {
                        ...
                        if (cacheJsonArray == null) {
                            if (getJsonArrayIndex(keyList[0]) <= tempJsonArray.length()) {
                                //在存储的jsonarray内取不到该index的jsonobject，如果该index刚好小于等于length，新建一个存储
                                if (keyList.length == 1) {
                                    try {
                                        if (isValidJsonObject(data)) {
                                            finalJsonArray.put(getJsonArrayIndex(keyList[0]), getJSONObjectFromString(data));
                                        } else {
                                            //不支持二维json数组，直接当字符串处理
                                            finalJsonArray.put(getJsonArrayIndex(keyList[0]), data);
                                        }
